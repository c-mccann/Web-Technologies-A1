...
var dir = "img/";
var imageNames = [];			// filled on document ready via ajax
var currentWidth;				// used to dynamically fill rows properly
var CLASS_NAME = "img";			// assigned to every image, used to remove borders from non selected images
...
function deleteShowMoreButton(){
	...
	if(showMoreButtonDiv){	// if not null , eg hasnt been removed yet
...
$(document).ready(function(){
	...
	$("#newrowbutton").click(function(){
		...
		myloop: for (var i = 0; i < currentImages.length; i++) {
			...
			var properImg = document.createElement('IMG');		// building the new image tag
			properImg.src = fullPath;
			properImg.width = currentWidth;
			properImg.id = id;
			properImg.class = CLASS_NAME;

			properImg.onclick = function() { borderDescription(this); };	//adding functions to events in the tag
